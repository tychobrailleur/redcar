      ...
      Menu::Builder.build do
        sub_menu "Plugins" do
          sub_menu "Strip Trailing Spaces", :priority => 195 do
            item "Enabled",           :command => ToggleStripTrailingSpaces, :type => :check, :checked => Proc.new { StripTrailingSpaces.enabled? }
            item "Strip Blank Lines", :command => ToggleStripBlankLines,     :type => :check, :checked => Proc.new { StripTrailingSpaces.strip_blank_lines? }
          end
        end
      end
    end
