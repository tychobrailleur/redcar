        ...
        sub_menu "Edit" do
          group(:priority => 20) do
           ...
           #                     :checked => lambda { tab and tab.respond_to?(:edit_view) and tab.edit_view.soft_tabs? }
              :checked => Proc.new { tab and tab.respond_to?(:edit_view) and tab.edit_view.soft_tabs? }
              ...
              sub_menu "Tab Width" do
                TabSettings::TAB_WIDTHS.each do |width|
                  ...
                  already_checker = Proc.new { tab and tab.respond_to?(:edit_view) and tab.edit_view.tab_width.to_s == width.to_s }
            ...
            sub_menu "Margin" do
              item "Word Wrap", :command => EditView::ToggleWordWrapCommand,
                                :type => :check, 
                                :checked => Proc.new { tab and tab.respond_to?(:edit_view) and tab.edit_view.word_wrap? }
              
              item "Show Margin", :command => EditView::ToggleShowMarginCommand,
                                  :type => :check, 
                                  :checked => Proc.new { tab and tab.respond_to?(:edit_view) and tab.edit_view.show_margin? }
  
              item Proc.new { tab && tab.respond_to?(:edit_view) ? "Margin Column: #{tab.edit_view.margin_column}" : "Margin Column" }, SetMarginColumnCommand
            end
