    ...
    def self.menus(window)
      Menu::Builder.build do
        ...
        sub_menu "View", :priority => 30 do
          ...
          group(:priority => :last) do
            separator
            item "Show Toolbar", :command => Application::ToggleToolbar, :type => :check, :checked => Proc.new { Redcar.app.show_toolbar? }
            item "Show Invisibles", :command => ToggleInvisibles, :type => :check, :checked => Proc.new { EditView.show_invisibles? }
            item "Show Line Numbers", :command => ToggleLineNumbers, :type => :check, :checked => Proc.new { EditView.show_line_numbers? }
          end
        end
        ...
        sub_menu "Help", :priority => :last do
          group(:priority => :last) do
            ...
            item "Check for Updates", :command => Application::ToggleCheckForUpdatesCommand, 
                                      :type => :check, 
                                      :checked => Proc.new { Application::Updates.check_for_updates? }
